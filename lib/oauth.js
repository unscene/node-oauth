...
Object.prototype.merge = function(b) {
  if (!b || !(b instanceof Object)) return this;
...
exports.createClient = function(port, host, secure, creds) {
  ...
  c.request = function(method, path, headers, body, signature) {
    ...
    //path = oauth.fillURL(path, host, port, secure);
...
Request.prototype.signRequest = function(method, path, headers, body, signature) {
  ...
  querystring.escape = encode_uri_rfc_3986;
  auth = querystring.stringify(oauth.normalize(auth),'\",','=\"');

  headers['Authorization'] = "OAuth " + auth + '\",oauth_signature=\"' + encode_uri_rfc_3986(base64) + '\"';
  return headers;
}
...
var encode_uri_rfc_3986 = function(str) {
...
Signature.prototype.key = function () {
  return [
    encode_uri_rfc_3986(this.consumer ? this.consumer.oauth_consumer_secret : ''),
    encode_uri_rfc_3986( this.token ? this.token.oauth_token_secret : '')
  ].join('&');
};
...
Signature.prototype.base_string = function(method, url, params) {
  querystring.escape = encode_uri_rfc_3986;
  return [
    method.toUpperCase(),
    encode_uri_rfc_3986(oauth.normalizeURL(url)),
    encode_uri_rfc_3986(querystring.stringify(oauth.normalize(params))),
  ].join('&');
};
...
HMAC.prototype.base = function(method,path,params) {
  return this.base_string(method, path, params);
};
...
HMAC.prototype.sign = function(method,path,params) {
  ...
  var base = this.base_string(method,path,params);
