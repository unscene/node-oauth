...
function signRequest(method, path, headers, body, signature, realm) {
	...
	else if (signature instanceof RSA) {
		base64 = signature.sign(method, path, joined);
	}
...
exports.createRsaConsumer = function(key, rsaPrivateKey) {
	var c = new Consumer();
	c.oauth_consumer_key = key;
...
util.inherits(RSA, Signature);

exports.RSA = RSA;

exports.createRsa = function (consumer, token) {
	var h = new RSA();
	h.algo = 'sha1';
	h.encoding = 'base64';
	h.name = 'RSA-SHA1';
...
RSA.prototype.base = function(method,path,params) {
	return this.baseString(method, path, params);
};

RSA.prototype.sign = function(method,path,params) {
	if (!this.consumer && !this.token) {
		throw new Error('Must provide a valid consumer or token');
	}

	var base = this.baseString(method, path, params);
	var signer = crypto.createSign(this.name);

	signer.update(base);
	var signed = signer.sign(this.consumer.rsaPrivateKey, this.encoding);
